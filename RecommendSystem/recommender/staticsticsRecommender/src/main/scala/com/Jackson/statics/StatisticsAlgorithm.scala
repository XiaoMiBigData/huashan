...
object staticsRecommender {
  ...
  def rateMoreRecently(spark: SparkSession)(implicit mongoConfig: MongoConfig): Unit = {
    ...
    val yearMonthOfRatings = spark.sql("select mid, score, changeDate(timstamp) as yearmonth from ratings")

    yearMonthOfRatings.createTempView("ymRatings")
  ...
  //按类别统计 平均分最高的Top10
  def genresTop10(spark: SparkSession)(movies: Dataset[MovieTest])(implicit mongoConfig: MongoConfig): Unit = {
